...
import static com.example.bonnie.mynewnotebook.R.layout.search_domain;
...
public class  MainActivity extends AppCompatActivity {
    ...
    public static void verifyStoragePermissions(Activity activity) {
        // Check if we have write permission 给手机读写权限
...
//创建ACTIVITY函数
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        ...
        switch(id){
            //添加单词
            case R.id.menu_add:
                interactiveInterface.inflateAddDialog();
                break;
            //考试模式
            case R.id.menu_exam:
...
//                menu_import();
                ...
                verifyStoragePermissions(MainActivity.this);//打开文件权限
    ...
    @Override
    public boolean onContextItemSelected(MenuItem item){//按住单词选中可以出现几个选择：删除，朗读等，底下有对应的几个函数
        ...
        switch (item.getItemId()) {
            case 0:
                ...
                interactiveInterface.inflateEditDialog(old_eng,old_fre,old_domain);
    ...
    //引入外部单词表的时候的读取文件，intent是传输类型的数据
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Uri uri = data.getData();//得到uri，后面就是将uri转化成file的过程。（地址）
